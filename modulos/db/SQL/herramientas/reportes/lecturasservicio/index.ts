...
export const getLecturasServicio = async (edif_id: String,serv_id:String[],usua_id:string,fecha: String,filter: any, orderBy: any, paginacion: any) => {

    const conexion = conexionDB();

    let _serv_id = (serv_id) ? "".concat("'{", serv_id.join(","), "}'::uuid[]") : "null::uuid[]"

    let query = conexion.from(db.raw(` mediciones.get_lecturas_servicios('${edif_id}',${_serv_id},'${usua_id}','${fecha}')`))
